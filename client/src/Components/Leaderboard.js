        ...
        ? communityUsers.map((user) => (
            <div key={user.id} className="flex w-full my-3">
              <div
                ...
                className="w-auto mr-6 font-bold text-5xl flex items-center"
              >
              ...
              <div className="flex flex-col w-full">
                ...
                <span className="text-md mb-2">
                  {user.points || 0} Items Collected
                </span>
                <div className="h-8 border rounded-full relative overflow-hidden w-full">
                  <div
                    style={{ width: findPercentage(user.points) }}
                    className="bg-gradient-to-r from-dark-green to-light-green h-full relative flex items-center"
                  >
                    <span className="absolute top-0 bottom-0 right-5 m-auto text-white font-bold h-min text-sm">
                      {user.points}
                    </span>
                  </div>
                </div>
